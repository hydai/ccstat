...
fn generate_usage_entries(count: usize) -> Vec<UsageEntry> {
    ...
    (0..count)
        .map(|i| {
            let timestamp = Utc.timestamp_opt(1704067200 + (i as i64 * 3600), 0).unwrap();
...
fn benchmark_aggregation(c: &mut Criterion) {
    ...
    let pricing_fetcher = rt.block_on(async {
        Arc::new(PricingFetcher::new(false).await)
    });
...
fn benchmark_cost_calculation(c: &mut Criterion) {
    ...
    group.bench_function("single_calculation", |b| {
        ...
        b.iter(|| {
            rt.block_on(async {
                calculator.calculate_cost(&tokens, &model).await.unwrap()
            })
        })
    });
...
fn benchmark_token_arithmetic(c: &mut Criterion) {
    ...
    group.bench_function("accumulated_addition_1k", |b| {
        ...
        b.iter(|| {
            tokens.iter().fold(TokenCounts::default(), |acc, t| acc + *t)
        })
    });
