...
fn benchmark_cost_calculation(c: &mut Criterion) {
    ...
    group.bench_function("calculate_from_pricing", |b| {
        ...
        b.iter(|| {
            let _cost = CostCalculator::calculate_from_pricing(
                black_box(&tokens),
                black_box(&pricing),
            );
        });
...
fn benchmark_cost_modes(c: &mut Criterion) {
    ...
    let pricing_fetcher = runtime.block_on(async {
        Arc::new(PricingFetcher::new(true).await)
    });
    ...
    group.bench_function("calculate_100_entries", |b| {
        ...
        b.iter(|| {
            ...
            for tokens in &entries {
                total_cost += CostCalculator::calculate_from_pricing(
                    black_box(tokens),
                    black_box(&pricing),
                );
            }
