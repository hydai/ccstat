...
use ccstat::{
    types::{UsageEntry, ISOTimestamp, ModelName, SessionId, TokenCounts},
...
impl UsageEntryBuilder {
    ...
    pub fn to_jsonl(self) -> String {
        let instance_field = self.instance_id
            .map(|id| format!(r#","uuid":"{}""#, id))
            .unwrap_or_default();
        
        let project_field = self.project
            .map(|p| format!(r#","cwd":"/home/user/{}""#, p))
            .unwrap_or_default();
        
        let cost_field = self.total_cost
...
pub async fn create_test_data_dir(entries: Vec<String>) -> (TempDir, DataLoader) {
    ...
    let loader = DataLoader::new().await.expect("Failed to create DataLoader");
    ...
    while current_date <= end_date {
        for session in 0..sessions_per_day {
            ...
            let entry = UsageEntryBuilder::new()
                .with_session_id(&format!("session-{}-{}", current_date.format("%Y%m%d"), session))
                .with_date(
                    current_date.year(),
                    current_date.month(),
                    current_date.day(),
                ...
                .with_model(model)
                .with_tokens(1000 + session as u64 * 100, 500 + session as u64 * 50)
                .with_project(project)
                .with_cost(0.01 * (session + 1) as f64)
                .to_jsonl();
